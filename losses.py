...
def sequential_set_no_stop_loss(scores, samples, oracle_ps, end_idx, self_teach_beta=1.0):
    ...
    for t in range(T):
        ...
        loss_ = F.kl_div(logp_policy, p_oracle, reduction='none').sum(1)    # KL-divergence loss

        losses.append(loss_)    # calulate one by one
...
def sequential_set_loss(scores, samples, oracle_ps, end_idx, self_teach_beta=1.0):
    ...
    for t in range(T):
        ...
        if self_teach_beta < 1.0:
            p_correct_policy = _correct_policy_distribution(scores[:, t, :], p_oracle)
            p_oracle = (1.0 - self_teach_beta)*p_correct_policy + self_teach_beta*p_oracle  # anneal
            p_oracle = (p_oracle / p_oracle.sum(1, keepdim=True)).detach()  # normalize
    ...
    # Generation ended when the number of end labels exceeds the number of labels
    end_indicator = ((samples == end_idx).cumsum(1) > (samples != end_idx).cumsum(1))   # torch.cumsum: cumulative sum, parameter is dimension (like axis in tf)
    # 1, 2, ..., t = 1 where t is the end time, t+1,...,T = 0.
    end_mask = (end_indicator.cumsum(1).cumsum(1) <= 1).float()

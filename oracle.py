...
class Oracle(object):
    def __init__(self, token_idxs, sample_dim, tok2i, i2tok, greedy=False):
        ...
        self._B = token_idxs.size(0)    # input sentences, build as root node
        self.tok2i = tok2i
        self.i2tok = i2tok
        self.trees = []
        invalid_behavior = 'split'
        for i in range(self._B):        # how many roots
    ...
    def valid_actions_vector(self):
        with th.no_grad():  # do not calculate gradients
            self._valid_actions.zero_() # fill this tensor with zeros
            for i in range(self._B):    # the current states for each tree
                if self._stopped[i]:
                    self._valid_actions[i][self.tok2i['<p>']] = 1
                    # the first index of _valid_actions is which tree, the second index is which token
                else:
                    ...
                    self._valid_actions[i].scatter_(0, valid_actions, 1)    # (dim, index, src)
    ...
    def update(self, samples):
        ...
        with th.no_grad():
            for i in range(self._B):
                if not self._stopped[i]:    # the i-th tree isn't stopped
                    self.trees[i].generate(samples[i].item())   # generate left or right child
                    self.trees[i].next()
        ...
        with th.no_grad():
            ...
            for i in range(self._B):
                if self._stopped[i]:
                    ...
                    left_action = self.trees[i].current.valid_actions[0]    # [0] means the left-most action
